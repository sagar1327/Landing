...
RUN apt-get install python3-catkin-tools && apt update && apt upgrade
RUN /bin/bash -c "cd ROS && \
                  rosdep -i install --from-path src/first_robot --rosdistro noetic -y && \ 
                  source /opt/ros/noetic/setup.bash && \
                  catkin build first_robot"
